...
// ListAccessKeysCommand creates the 'objectstorage user list-access-keys' command
func ListAccessKeysCommand() commands.Command {
	return &listAccessKeysCommand{
		BaseCommand: commands.New(
			"list-access-keys",
			"List access keys for a user in managed object storage service",
			"upctl object-storage list-access-keys <service-uuid> --username myuser",
			"upctl object-storage list-access-keys my-service --username myuser",
		),
	}
}

type listAccessKeysCommand struct {
	*commands.BaseCommand
	params request.GetManagedObjectStorageUserAccessKeysRequest
}

// InitCommand implements Command.InitCommand
func (s *listAccessKeysCommand) InitCommand() {
...
func (s *listAccessKeysCommand) MaximumExecutions() int {
	return 1
}

// Execute implements commands.MultipleArgumentCommand
func (s *listAccessKeysCommand) Execute(exec commands.Executor, uuid string) (output.Output, error) {
