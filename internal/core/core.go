...
To be able to manage your UpCloud resources, you need to configure credentials for ` + "`" + `upctl` + "`" + ` and enable API access for these credentials. We recommend using API tokens: configure in ` + "`" + `~/.config/upctl.yaml` + "`" + ` or set the ` + "`" + `UPCLOUD_TOKEN` + "`" + ` environment variable. Alternatively, use ` + "`" + `UPCLOUD_USERNAME` + "`" + ` and ` + "`" + `UPCLOUD_PASSWORD` + "`" + ` environment variables. API access can be configured on the Account page of the UpCloud Hub. We recommend you to set up a sub-account specifically for the API usage with its own credentials, as it allows you to assign specific permissions for increased security.`
